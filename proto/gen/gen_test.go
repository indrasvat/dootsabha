...
func TestInvokeRequestConstruction(t *testing.T) {
	...
	if req.GetModel() != "claude-opus-4-8" {
		t.Errorf("Model = %q, want %q", req.GetModel(), "claude-opus-4-8")
	}
...
func TestInvokeResponseConstruction(t *testing.T) {
	...
	if resp.GetModel() != "claude-opus-4-8" {
...
func TestCapabilitiesResponseConstruction(t *testing.T) {
	resp := &gen.CapabilitiesResponse{
		...
		SupportedModels:   []string{"claude-opus-4-8", "claude-haiku-4-5"},
	...
	if resp.GetSupportedModels()[0] != "claude-opus-4-8" {
		t.Errorf("SupportedModels[0] = %q", resp.GetSupportedModels()[0])
	}
	if resp.GetDefaultModel() != "claude-opus-4-8" {
		t.Errorf("DefaultModel = %q", resp.GetDefaultModel())
	}
	if resp.GetMaxContextTokens() != 1000000 {
