#!/usr/bin/env bash
# L4 motion evidence for task 706 — a real grok-4.6 council, captured as video.
#
...
# ffmpeg stitch the frames. What you see is genuinely grok-4.6 working, not a
# replayed transcript.
set -euo pipefail
...
export XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR_OVERRIDE:-/tmp/shux706v}"
...
PROMPT='In exactly two sentences: why must a default model id declared in four separate places be bumped atomically?'

echo "Recording a live council (codex, agy, grok — chair grok) …"
shux session create "$SESS" -d --title council --cwd "$REPO" \
  -- bash -lc "cd '$REPO' && clear && $BIN council --agents codex,agy,grok --chair grok '$PROMPT'; echo; echo __FRAME_DONE__; sleep 600" \
  >/dev/null
...
ffmpeg -y -loglevel error -framerate 8 -pattern_type glob -i "$FRAMES/*.png" \
  ...
  "$OUT/grok-4-6-council.mp4"

ffmpeg -y -loglevel error -framerate 8 -pattern_type glob -i "$FRAMES/*.png" \
  -vf "fps=8,scale=1000:-1:flags=lanczos,split[a][b];[a]palettegen[p];[b][p]paletteuse" \
  "$OUT/grok-4-6-council.gif"

du -h "$OUT/grok-4-6-council.mp4" "$OUT/grok-4-6-council.gif"
