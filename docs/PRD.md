...
| **Spike before you build** | Every technical assumption validated with throwaway code | Phase 0 exists entirely for spikes — 7 disposable programs |
...
|------|------------|------------|---------------|
| 1.1 | Project scaffold + Makefile + CLAUDE.md | — | — |
...
|-------|------|-------|-----------|--------|----------|
| **L1** | Compile + lint + `go vet` | <5s | Every save | N/A | No |
| **L2** | Unit tests (`go test ./...`) | <2s | Every change | Yes (all deps mocked) | No |
| **L3** | Smoke tests (binary execution) | <10s | Every build | Mock provider scripts | No |
| **L4** | Integration tests (real CLIs) | 30-60s | Every phase | No (real CLIs) | ~$0.05 |
| **L5** | Acceptance + visual (iTerm2-driver) | 2-5min | Phase gate | No (real CLIs + iTerm2) | ~$0.50 |

### 10.2 Make Targets

```makefile
make test         # L2: unit tests (fast, mocked)
...
A tiny bash script mimicking agent CLI behavior for offline testing:

```bash
#!/usr/bin/env bash
# mock-claude: simulates claude CLI for smoke tests
while [[ $# -gt 0 ]]; do
  case $1 in
    -p) PROMPT="$2"; shift 2 ;;
...
if [ "$FORMAT" = "json" ]; then
  echo '{"result":"Mock: '$PROMPT'","session_id":"mock_123","cost_usd":0.001,"model":"'${MODEL:-sonnet-4-6}'"}'
else
  echo "Mock response to: $PROMPT"
fi
```

### 10.4 Critical Testing Rules
